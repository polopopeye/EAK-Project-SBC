...
if(autotakeprofit==true){
  ...
  if(DiffPipsResistanceT<sensorautotkpf&&profitordersV>0&&DiffPipsv>minimumpipsallowedauto){
     ...
     string sfileHandlelucky = FileOpen(fileNamelucky,FILE_CSV|FILE_READ);// | FILE_WRITE
...
if(gestionlotesmethod==3){
  if(posicionesabiertasrobotc==0){
    lotesposicionc=NormalizeDouble((lotesinicial*riskextrapolatorc*avramisCL*cFundamentalB*cluckyB),lotround);
  }else{
    if(LastOrderlotsc>0){
      if(LastOrderlotsc=>maxlots){
      lotesposicionc=NormalizeDouble(maxlots,lotround);
      }else{
        lotesposicionc=NormalizeDouble((LastOrderlotsc*multiplicador*riskextrapolatorc*avramisCL*cluckyB*cFundamentalB),lotround);
      }
  ...
  if(posicionesabiertasrobotv==0){
    lotesposicionv=NormalizeDouble((lotesinicial*riskextrapolatorv*avramisVL*cFundamentalS*cluckyS),lotround);
  }else{
    if(LastOrderlotsv>0){
      if(LastOrderlotsv=>maxlots){
        lotesposicionv=NormalizeDouble(maxlots,lotround);
      }
      lotesposicionv=NormalizeDouble((LastOrderlotsv*multiplicador*riskextrapolatorv*avramisVL*cluckyS*cFundamentalS),lotround);//+posicionesabiertasrobotv);
    }else{
      lotesposicionv=NormalizeDouble((lotesinicial*riskextrapolatorv*avramisVL*cFundamentalS*cluckyS),lotround);
    }
  }
...
if(secureorders==true){
    ...
    if(posicionesabiertasrobotc>priorizarposiciones){
      if(lotesposicionc<=lotesinicial){
    ...
    if(posicionesabiertasrobotv>priorizarposiciones){
      if(lotesposicionv<=lotesinicial){
