...
//-------------------------------------------- EXTERNAL VARIABLE ---------------------------------------------
//------------------------------------------------------------------------------------------------------------
extern string MainC="USD";
extern string SecC="JPY";
extern bool    ReportActive      = true;                // Report for active chart only (override other inputs)
 bool    IncludeHigh       = true;                 // Include high
 bool    IncludeMedium     = true;                 // Include medium
 bool    IncludeLow        = true;                 // Include low
 bool    IncludeSpeaks     = true;                 // Include speaks
 ...
 int     UpdateHour        = 1;                    // Update every (in hours)
input string   lb_0              = "";                   // ------------------------------------------------------------
...
//---
   ...
   string sUrl="http://cdn-nfs.forexfactory.net/ff_calendar_thisweek.xml";
...
//____________________EDITOR CODIGO
double MainR=0;
double SecR=0;
double usdfx=0;
double eurfx=0;
...
if(eCountry[i]=="USD"){
  if(eForecast[i]>ePrevious[i]){
    if(eImpact[i]=="High"){
      usdfx=usdfx+(1*highfx);
    }
    if(eImpact[i]=="Medium"){
      usdfx=usdfx+(1*medfx);
    }
...
if(MainC=="USD")MainR=usdfx;//L
if(MainC=="EUR")MainR=eurfx;//L
if(MainC=="JPY")MainR=jpyfx;//L
if(MainC=="AUD")MainR=audfx;//L
if(MainC=="GBP")MainR=gbpfx;//L
if(MainC=="CAD")MainR=gbpfx;
if(MainC=="CHF")MainR=gbpfx;
if(MainC=="NZD")MainR=gbpfx;
if(SecC=="USD")SecR=usdfx;//L
if(SecC=="EUR")SecR=eurfx;//L
if(SecC=="JPY")SecR=jpyfx;//L
if(SecC=="AUD")SecR=audfx;//L
if(SecC=="GBP")SecR=gbpfx;//L
if(SecC=="CAD")SecR=gbpfx;
if(SecC=="CHF")SecR=gbpfx;
if(SecC=="NZD")SecR=gbpfx;

int TendFund=0;
if(MainR>SecR&&MainR>0){
  TendFund=1;
}
if(MainR<SecR&&SecR>0){
  TendFund=2;
}
...
//____________________________
      ...
      if(ShowPanel)
        {
          string fileNamefund=MainC+SecC+"fundamental.txt";
          if(TendFund==1){
            string fileHandle6fund = FileOpen(fileNamefund,FILE_CSV|FILE_READ | FILE_WRITE);//|FILE_READ| FILE_WRITE
             if(fileHandle6fund==INVALID_HANDLE){
           printf("Handle Invalido, ERROR 6 fund");
            }else{
              string data6fund = "COMPRA";
                      FileWrite(fileHandle6fund,data6fund);
                      FileClose(fileHandle6fund);
               }
          }
          if(TendFund==2){
            string fileHandle5fund = FileOpen(fileNamefund,FILE_CSV|FILE_READ | FILE_WRITE);//|FILE_READ| FILE_WRITE
             if(fileHandle5fund==INVALID_HANDLE){
           printf("Handle Invalido, ERROR 5 fund");
            }else{
              string data5fund = "VENTA";
                      FileWrite(fileHandle5fund,data5fund);
                      FileClose(fileHandle5fund);
               }
          }
