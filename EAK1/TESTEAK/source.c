...
extern int cogind=true;
extern int coguniqueid=744;
...
extern string SENTIMENT="SENTIMENT INDICATOR";
extern bool sentimentind=true;
//extern bool sentimentkeepposition=true;
//extern bool sentimentdinamicgrid=true;
//extern bool powersentiment=true;
...
if(cogind==true){
  double cognumberz = iCustom(Symbol(),cogtiempo,"COG_H",coguniqueid,125,2,2.0,false,true,0,0);
  double cognumberh = iCustom(Symbol(),cogtiempo,"COG_H",coguniqueid,125,2,2.0,false,true,8,0);
  double cognumberl = iCustom(Symbol(),cogtiempo,"COG_H",coguniqueid,125,2,2.0,false,true,9,0);
...
ObjectSetText(Symbol()+"cog","COG - C:"+cocientecogC+" - V:"+cocientecogV,7,NULL,Orange);
...
if(sentimentind==true){
  double sentiment = iCustom(Symbol(),PERIOD_M1,"TraderSentiment","EAK",fxusername,fxpassword,"EAK",true,true,true,true,true,5,0);
string tendsentiment="ERROR";
double cocientesentimentcompra=0;
double cocientesentimentventa=0;
double cocientecompra=0;
double cocienteventa=0;
double opesentimentcompra=0;
double opesentimentventa=0;
double ventapositivosentiment=100 - sentiment;


if(sentiment>53||sentiment==53){
  tendsentiment="COMPRA";
 cocientecompra=1;
 cocienteventa=0;
 cocientesentimentcompra=0.01;
 cocientesentimentventa=0.008;

opesentimentcompra=sentiment*cocientesentimentcompra+cocientecompra;
opesentimentventa=sentiment*cocientesentimentventa+cocienteventa;
  ObjectSetText(Symbol()+"sentiment","COMPRA -"+sentiment+" - C:"+opesentimentcompra+"- V:"+opesentimentventa,7,NULL,Blue);
    ObjectSet(Symbol()+"sentiment",OBJPROP_CORNER,0);
    ObjectSet(Symbol()+"sentiment",OBJPROP_XDISTANCE,15);
    ObjectSet(Symbol()+"sentiment",OBJPROP_YDISTANCE,150);
}
if(sentiment>47&&sentiment<53){
    opesentimentcompra=0.6;
    opesentimentventa=0.6;
    ObjectSetText(Symbol()+"sentiment","NEUTRA -"+sentiment+" - C:"+opesentimentcompra+"- V:"+opesentimentventa,7,NULL,Orange);
      ObjectSet(Symbol()+"sentiment",OBJPROP_CORNER,0);
      ObjectSet(Symbol()+"sentiment",OBJPROP_XDISTANCE,15);
      ObjectSet(Symbol()+"sentiment",OBJPROP_YDISTANCE,150);
}
if(sentiment<47||sentiment==47){
 cocienteventa=1;
 cocientecompra=0;
 cocientesentimentventa=0.01;
 cocientesentimentcompra=0.008;
  tendsentiment="VENTA";

  opesentimentcompra=ventapositivosentiment*cocientesentimentcompra+cocientecompra;
  opesentimentventa=ventapositivosentiment*cocientesentimentventa+cocienteventa;

  ObjectSetText(Symbol()+"sentiment","VENTA - "+sentiment+" - C:"+opesentimentcompra+"- V:"+opesentimentventa,7,NULL,Red);
    ObjectSet(Symbol()+"sentiment",OBJPROP_CORNER,0);
    ObjectSet(Symbol()+"sentiment",OBJPROP_XDISTANCE,15);
    ObjectSet(Symbol()+"sentiment",OBJPROP_YDISTANCE,150);
}

//printf("opesentimentcompra"+opesentimentcompra+"-"+opesentimentventa);
//KEEP POSITION DESCONTINUADO--------------------------
//if(sentimentkeepposition==true){
//  int keepordersentimentC=1;
//  int keepordersentimentS=1;
//  if(sentiment>50){
//  keepordersentimentC=1;
//  keepordersentimentS=0;
//  }else{
//  keepordersentimentC=0;
//  keepordersentimentS=1;
//  }
//}
if(sentiment==NULL){
  ObjectSetText(Symbol()+"sentiment","ERROR DE OBTENCION DE DATOS",7,NULL,Red);
    ObjectSet(Symbol()+"sentiment",OBJPROP_CORNER,0);
    ObjectSet(Symbol()+"sentiment",OBJPROP_XDISTANCE,15);
    ObjectSet(Symbol()+"sentiment",OBJPROP_YDISTANCE,150);
}
...
if(dinamicgridX==true){
  double Volatilidad= iCustom(Symbol(),PERIOD_CURRENT,"BetterVolume",40,"EAK",10,20,6,0);

double dinamicgridC = Volatilidad/10;
double dinamicgridV = Volatilidad/10;

if(sentiment<50&&dinamicopen==true){
...
if(dinamicclose==true){
  ...
  Cminpositionallowedtoclosepipsa=Volatilidad*1.5*keeppositionv2C;
  Cminpositionallowedtoclosepipsb=Volatilidad*1*keeppositionv2C;
  Cminpositionallowedtoclosepipsc=Volatilidad*0.8*keeppositionv2C;
  Vminpositionallowedtoclosepipsa=Volatilidad*1.5*keeppositionv2V;
  Vminpositionallowedtoclosepipsb=Volatilidad*1*keeppositionv2V;
  Vminpositionallowedtoclosepipsc=Volatilidad*0.8*keeppositionv2V;
}else{
...
ObjectSetText(Symbol()+"dinamicgrid","D.G.C: "+gridpositionC+"- D.G.V: "+gridpositionV+"- MPCCA:"+Cminpositionallowedtoclosepipsa+"- MPCVA:"+Vminpositionallowedtoclosepipsa ,9,NULL,Orange);
...
if(OrderType()==OP_SELL){
 ...
 if(tiempoelapsedS<ordersElapsedA){//MENOS DE UN DIA
  // if(keepordersentimentS==1&&sentimentdinamicgrid==true){
  //   gridpositionV=gridposition / 2.5;
  // }else{
...
if(tiempoelapsedB<ordersElapsedA){//MENOS DE UN DIA
  //if(keepordersentimentC==1&&sentimentdinamicgrid==true){
//    gridpositionC=gridposition / 2.5;
//  }else{
...
if(sentimentind==true){
     ...
     if(security==true){
       ...
       double creditpercentA = AccountBalance()*0.1;
       double balancepercentA = AccountBalance()*0.1;
       double creditpercentB = AccountBalance()*-0.1;
       double balancepercentB = AccountBalance()*-0.1;
...
if(gestionlotesmethod==3){
  if(posicionesabiertasrobotc==0){
    lotesposicionc=NormalizeDouble((lotesinicial*riskextrapolatorc*avramisCL*cFundamentalB*cluckyB*csimeterB*opesentimentcompra),lotround);
  }else{
    if(LastOrderlotsc>0){
      if(LastOrderlotsc>maxlots||LastOrderlotsc==maxlots){
      lotesposicionc=NormalizeDouble(maxlots,lotround);
      }else{
        lotesposicionc=NormalizeDouble((LastOrderlotsc*multiplicador*riskextrapolatorc*avramisCL*cluckyB*csimeterB*cFundamentalB*opesentimentcompra),lotround);
      }
  ...
  if(posicionesabiertasrobotv==0){
    lotesposicionv=NormalizeDouble((lotesinicial*riskextrapolatorv*avramisVL*cFundamentalS*cluckyS*csimeterS*opesentimentventa),lotround);
  }else{
    if(LastOrderlotsv>0){
      if(LastOrderlotsv>maxlots||LastOrderlotsv==maxlots){
        lotesposicionv=NormalizeDouble(maxlots,lotround);
      }
      lotesposicionv=NormalizeDouble((LastOrderlotsv*multiplicador*riskextrapolatorv*avramisVL*cluckyS*csimeterS*cFundamentalS*opesentimentventa),lotround);//+posicionesabiertasrobotv);
    }
    //else{
...
//printf("lotesC"+lotesposicionc+"-"+lotesposicionv);
if(debugoption==true){
