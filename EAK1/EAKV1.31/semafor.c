...
extern string NOTRADE="27 MArket para FBS";
extern bool CurrenciesWindowBelowTable = FALSE;
extern string accounttype="m";
...
//string gsa_188[8] = {"USD", "EUR", "GBP", "CHF", "JPY", "CAD", "AUD"};
...
//string gsa_196[27] = {"EURUSD", "GBPUSD", "AUDUSD", "USDJPY", "USDCHF", "USDCAD", "EURAUD", "EURCAD", "EURCHF", "EURGBP", "EURJPY", "GBPJPY", "GBPCHF", "NZDUSD", "AUDCAD", "AUDJPY", "CHFJPY",
//      "AUDNZD", "NZDJPY", "NZDCAD", "NZDCHF", "EURNZD", "GBPCAD", "GBPAUD", "AUDCHF", "CADCHF", "CADJPY"};
//Normal
...
void init() {
   ...
   for (int index_0 = 0; index_0 < 27; index_0++) {
      symbol_8 = gsa_196[index_0]+accounttype;
      if (StringLen(SymbolPrefix) > 1) {
         gi_264 = TRUE;
         if (StringFind(SymbolPrefix, "|") == 0) {
            gs_256 = StringSubstr(SymbolPrefix, 1);
            symbol_8 = symbol_8 + gs_256;
            gi_268 = -StringLen(gsa_196[index_0]+accounttype);
         }
      ...
      if (MarketInfo(symbol_8, MODE_POINT) == 0.0) ls_24 = ls_24 + ":" + gsa_196[index_0]+accounttype;
      else {
...
void start() {
   ...
   for (int index_0 = 0; index_0 < index_4; index_0++) {
      ...
      if (StringFind(DontShowThisPairs, gsa_196[index_0]+accounttype) < 0) {
         ...
         if (li_40 >= 0) {
            f0_8(index_0 + "sLED");
            string fileNamesimeter=StringSubstr(gsa_204[li_40], 0, 6)+"simeter.txt";
            if (gda_212[index_0][0] < 0.0) {
              if(gda_212[index_0][0]<-80){
              //  printf("Vender 1"+gsa_204[li_40]);
                //----------------------------------
                string fileHandlesimeter2 = FileOpen(fileNamesimeter,FILE_CSV|FILE_READ | FILE_WRITE);//|FILE_READ| FILE_WRITE
                 if(fileHandlesimeter2==INVALID_HANDLE){
                printf("Handle Invalido, ERROR1 Simeter");
                }else{
                  string datasimeter2 = "VENTA";
                          FileWrite(fileHandlesimeter2,datasimeter2);
                          FileClose(fileHandlesimeter2);
                   }
                //---------------------------------
              }else{
                //printf("Dejar"+gsa_204[li_40]);
                //----------------------------------
                string fileHandlesimeter3 = FileOpen(fileNamesimeter,FILE_CSV|FILE_READ | FILE_WRITE);//|FILE_READ| FILE_WRITE
                 if(fileHandlesimeter3==INVALID_HANDLE){
                printf("Handle Invalido, ERROR3 Simeter");
                }else{
                  string datasimeter3 = "NEUTRO";
                          FileWrite(fileHandlesimeter3,datasimeter3);
                          FileClose(fileHandlesimeter3);
                   }
                //---------------------------------
              }
               li_28 = 2;
              ...
              if(gda_212[index_0][0]>80){
                //printf("Comprar 1"+gsa_204[li_40]);
                //----------------------------------
                string fileHandlesimeter1 = FileOpen(fileNamesimeter,FILE_CSV|FILE_READ | FILE_WRITE);//|FILE_READ| FILE_WRITE
                 if(fileHandlesimeter1==INVALID_HANDLE){
                printf("Handle Invalido, ERROR11 Simeter");
                }else{
                  string datasimeter1 = "COMPRA";
                          FileWrite(fileHandlesimeter1,datasimeter1);
                          FileClose(fileHandlesimeter1);
                   }
                //---------------------------------
              }else{
                //----------------------------------
                string fileHandlesimeter4 = FileOpen(fileNamesimeter,FILE_CSV|FILE_READ | FILE_WRITE);//|FILE_READ| FILE_WRITE
                 if(fileHandlesimeter4==INVALID_HANDLE){
                printf("Handle Invalido, ERROR4 Simeter");
                }else{
                  string datasimeter4 = "NEUTRO";
                          FileWrite(fileHandlesimeter4,datasimeter4);
                          FileClose(fileHandlesimeter4);
                   }
                //---------------------------------
              }
               li_28 = 0;
...
double f0_5(string as_0) {
   ...
   for (int index_32 = 0; index_32 < ArraySize(gsa_204); index_32++) {
      ...
      if (as_0 == StringSubstr(ls_40, 0, 3) || as_0 == StringSubstr(ls_40, 3, 3)) {
         ...
         if (ld_56 > 75.0){
           //printf("ALGO"+ls_40+ld_56);
           li_36 = 7;
         }
         if (ld_56 > 90.0){
           //printf("ALGO1"+ls_40+ld_56);
           li_36 = 8;
         }
         if (ld_56 > 97.0){
           li_36 = 9;
        //   printf("ALGO2"+ls_40+ld_56);
         }
         count_8++;
