  ...
  for (int i=0; i<ccCP; i++)   {
    ...
    for (int j=0; j<8; j++)   {
      if (ccy_name[j] == temp)  {
        ...
        ccy_count[j]    += 1;
//printf("ccy_name[j]"+ccy_name[j]+"ccy_strength[j]"+ccy_strength[j]+"ccy_count[j]"+ccy_count[j]);
        break;
    } }
  ...
  string fileNamecurrency="CURRENCY.txt";                                                                   //   string, it has priority, but everything in the string gets sorted
  ...
  for (j=0; j<8; j++)
  {
      ...
      if( ccy_name[j] == QuoteCur )
      {
       ...
       fileNamecurrency=ccy_name[j]+"currency.txt";
       //----------------------------------
       string fileHandlecurrency = FileOpen(fileNamecurrency,FILE_WRITE);//|FILE_READ| FILE_WRITE
        if(fileHandlecurrency==INVALID_HANDLE){
       printf("Handle Invalido, ERROR1 currency");
       }else{
         string datacurrency = out_value;
                 FileWrite(fileHandlecurrency,datacurrency);
                 FileClose(fileHandlecurrency);
          }
          //printf(fileNamecurrency+datacurrency+"tstr1"+tstr1);
...
//    string objname = IndiName + "-" + j;
//printf("ccy_name[j]"+ccy_name[j]+"out_value"+out_value);
