...
extern bool currencyv2=true;
//extern string SENTIMENT="SENTIMENT INDICATOR";
//extern bool sentimentind=false;
...
double cocientesentimentcompra=1;
double cocientesentimentventa=1;
double cocientecompra=1;
double cocienteventa=1;
double opesentimentcompra=1;
double opesentimentventa=1;



if(sentimentind==true){
double sentiment = iCustom(Symbol(),PERIOD_M1,"TraderSentiment","EAK",fxusername,fxpassword,"EAK",false,false,false,true,false,5,0);
...
if(sentiment<110){
  ...
  if(sentiment>=53&&sentiment<110){
    tendsentiment="COMPRA";
   cocientecompra=0.86;
   cocienteventa=0;
   cocientesentimentcompra=0.0099;
  ...
  opesentimentventa=sentiment*cocientesentimentventa+cocienteventa;
    ObjectSetText(Symbol()+"sentiment","SentimentC -"+sentiment+" c:"+opesentimentcompra+" v:"+opesentimentventa,7,NULL,Blue);
  ...
  if(sentiment>47&&sentiment<53){
      ...
      ObjectSetText(Symbol()+"sentiment","SentimentN -"+sentiment+" c:"+opesentimentcompra+" v:"+opesentimentventa,7,NULL,Orange);
        ObjectSet(Symbol()+"sentiment",OBJPROP_CORNER,0);
        ObjectSet(Symbol()+"sentiment",OBJPROP_XDISTANCE,15);
        ObjectSet(Symbol()+"sentiment",OBJPROP_YDISTANCE,150);
  }

  if(sentiment<=47&&sentiment<0){
   cocienteventa=0.86;
   cocientecompra=0;
   cocientesentimentventa=0.0099;
   cocientesentimentcompra=0.008;
    ...
    ObjectSetText(Symbol()+"sentiment","SentimentV - "+sentiment+" c:"+opesentimentcompra+" v:"+opesentimentventa,7,NULL,Red);
      ObjectSet(Symbol()+"sentiment",OBJPROP_CORNER,0);
      ObjectSet(Symbol()+"sentiment",OBJPROP_XDISTANCE,15);
      ObjectSet(Symbol()+"sentiment",OBJPROP_YDISTANCE,150);
  }
...
if(sentiment==NULL||sentiment>1000||sentiment<0){
  ObjectSetText(Symbol()+"sentiment","ERROR OBTENCION SENT - "+sentiment,7,NULL,Red);
    ObjectSet(Symbol()+"sentiment",OBJPROP_CORNER,0);
    ObjectSet(Symbol()+"sentiment",OBJPROP_XDISTANCE,15);
    ObjectSet(Symbol()+"sentiment",OBJPROP_YDISTANCE,150);
}
...
if(fundamentalcal==true){
  ...
  string sfileHandlefundamental = FileOpen(fileNamefundamental,FILE_READ);// | FILE_WRITE
