...
if(gestionlotesmethod==3){
  if(posicionesabiertasrobotc==0){
    lotesposicionc=NormalizeDouble((lotesinicial*riskextrapolatorc*avramisCL),lotround);
  }else{
    if(LastOrderlotsc>0){
      if(LastOrderlotsc>maxlots||LastOrderlotsc==maxlots){
      lotesposicionc=NormalizeDouble(maxlots,lotround);
      }else{
        lotesposicionc=NormalizeDouble((LastOrderlotsc*multiplicador*riskextrapolatorc*avramisCL),lotround);
      }
  ...
  if(posicionesabiertasrobotv==0){
    lotesposicionv=NormalizeDouble((lotesinicial*riskextrapolatorv*avramisVL),lotround);
  }else{
    if(LastOrderlotsv>0){
      if(LastOrderlotsv>maxlots||LastOrderlotsv==maxlots){
        lotesposicionv=NormalizeDouble(maxlots,lotround);
      }
      lotesposicionv=NormalizeDouble((LastOrderlotsv*multiplicador*riskextrapolatorv*avramisVL),lotround);//+posicionesabiertasrobotv);
    }
