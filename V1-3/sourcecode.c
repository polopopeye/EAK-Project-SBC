...
//------------------------------------------------------------------
double takeprofitp1v = iCustom(Symbol(),30,"SL_BW",0,0);//15 //30
 ...
 int tiempo2 = TimeCurrent()-OrderOpenTime();
     ...
     posicionesabiertasc++;
                 ...
                 if(cierreposicion==0){
                   ...
                   if(tiempo2>604800){
                     if(OrderTakeProfit()<restop2){
                     printf("takeprofit es mas pequeño (resistencia top valor mas alto) que la resistencia top actual va en camino bueno...");
                      if(OrderTakeProfit()==0){
                         OrderModify(OrderTicket(),0,0,restop2,0,Blue);
                    printf("takeprofit no establecido, establecerlo por primera vez en "+restop2);
                      }
                     }else{
                       OrderModify(OrderTicket(),0,0,restop2,0,Blue);
                       printf("takeprofit es mas alto que el nuevo resbot "+restop2);
                     }
