...
// Setting MSK time (Moscow time)to Alpari server's time
...
//----
   ...
   int maxind=MathRound((Hour_Start_1*60+Minute_Start_1)/Period());
//----
   int counted_bars=IndicatorCounted();
   if(counted_bars<0) return(-1);
   if(counted_bars>0) counted_bars--;
   int limit=Bars-counted_bars;
   if(counted_bars==0) limit-=1+maxind;
...
//----
   ...
   if(IIIuPuHa_KaHaJIa>MuH_IIIuPuHa_KaHaJIa)
     {BbIBog=", Buy from "+DoubleToStr(HighKaHaJI,4)+", Sell from "+DoubleToStr(LowKaHaJI,4);}
   else {BbIBog=", not trading";}
   Comment("\n","\n","Working server's time: since ",TextHour_Start_1,":",TextMinute_Start_1," till ",TextHour_End_1,":",TextMinute_End_1,
           " and since ",TextHour_Start_2,":",TextMinute_Start_2," до ",TextHour_End_2,":",TextMinute_End_2,
           "\n","Width of the channel: ",IIIuPuHa_KaHaJIa,BbIBog);
//----
   return(0);
  }
//+------------------------------------------------------------------+
