...
//   ObjectCreate(Symbol()+6,OBJ_LABEL,0,0,0,0); //resistencia top a corto plazo
...
//+------------------------------------------------------------------+
//   Resistances para abrir posiciones
//+------------------------------------------------------------------+

//double resabrircompra = iCustom(Symbol(),2,"SL_BW",0,0); //resistencia bot
//double resabrirventa = iCustom(Symbol(),2,"SL_BW",1,0); //resistencia top

double abrirposicion = iCCI(Symbol(),1,100,PRICE_MEDIAN,0);
...
//     }
   ...
   if(TotalOrdersCount()>=0&&TotalOrdersCount()<198)
     {
...
if(sentimientobandas==1&&sentimientocci==1){
  if(Ask<Bandabajo3m1){
    OrderSend(Symbol(),OP_BUY,Lots,Ask,Slippage,0,0,restop,MagicNumber,0,Blue); //Compra 

 if(abrirposicion<100){
//es mas grande por lo que existe y no hay que abrir
 }
  }
}
if(sentimientobandas==2&&sentimientocci==2){
  if(Ask>Bandaalto3m1){
    OrderSend(Symbol(),OP_SELL,Lots,Bid,Slippage,0,0,resbot,MagicNumber,0,Red); //Venta
